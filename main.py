...
def run(
    ...
    run_save_path = utils.create_storage_folder(
        results_path, log_project, log_group, run_name, mode="iterate"
    )
    ...
    for dataloader_count, dataloaders in enumerate(list_of_dataloaders):
        ...
        for i, SimpleNet in enumerate(simplenet_list):
            ...
            if not test:
                ...
                # If test flag is set, run evaluation using existing checkpoints.
                # SimpleNet.test currently returns (auroc, full_pixel_auroc).
                # We set pro_auroc to -1 to indicate it's not computed here.
                i_auroc, p_auroc = SimpleNet.test(dataloaders["training"], dataloaders["testing"], save_segmentation_images)
                pro_auroc = -1
